...
export function Avatar({ mood, speaking = false, name, size = 64 }: Props) {
  ...
  return (
    <div ref={wrapRef} className="flex flex-col items-center gap-1.5 select-none">
      <div className="aura-ring">
        <div
          key={mood}
          className="rounded-full relative mood-fade"
          style={{
            ...
            animation: "caissa-breathe 4s ease-in-out infinite, caissa-pop 320ms ease-out",
          }}
        >
          <svg viewBox="0 0 100 100" width={size} height={size} className="absolute inset-0">
            <path d={`M 28 ${36 + m.browY} Q 36 ${30 + m.browY + m.browTilt} 44 ${36 + m.browY - m.browTilt}`}
              stroke="oklch(0.22 0.005 60)" strokeWidth={2.4} strokeLinecap="round" fill="none" />
            <path d={`M 56 ${36 + m.browY - m.browTilt} Q 64 ${30 + m.browY + m.browTilt} 72 ${36 + m.browY}`}
              stroke="oklch(0.22 0.005 60)" strokeWidth={2.4} strokeLinecap="round" fill="none" />
          ...
          {mood === "thinking" && (
            <span className="absolute -top-1 -right-1 mono text-[10px] px-1.5 py-0.5 rounded-full bg-foreground text-background animate-pulse z-10">…</span>
          )}
        </div>
      </div>
      {name && <div className="serif text-xs leading-tight">{name}</div>}
      <style>{`
        @keyframes caissa-breathe {
          0%, 100% { transform: scale(1) perspective(400px); }
          50% { transform: scale(1.025) perspective(400px); }
        }
      `}</style>
    </div>
  );
